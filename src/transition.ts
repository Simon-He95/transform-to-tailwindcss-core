...
export function transition(key: string, val: string) {
  ...
  if (times.includes(key)) return `${key.split('-')[1]}-${value.slice(0, -2)}`
}
    ...
    .map((item) => {
      if (/^[0-9]/.test(item) || /^\.[0-9]/.test(item)) {
        const calculateTime = item.endsWith('s')
          ? 1000 * +item.slice(0, -1)
          : item.slice(0, -3)
        if (hasDuration) return `${important}delay-${calculateTime}`
        hasDuration = true
        return `${important}duration-${calculateTime}`
      }
      if (item.startsWith('background')) return `${important}transition-colors`
      if (item === 'linear') return `${important}ease-linear`
      if (/^(cubic-bezier)|(ease)/.test(item)) return `${important}${item}`
